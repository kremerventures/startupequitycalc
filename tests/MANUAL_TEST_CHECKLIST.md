...
11. Install the PWA, test it offline, reconnect, and confirm the v18 cache update appears.
19. **Install button.** On **Android Chrome** an **Install app** pill appears in the header once Chrome deems the site installable; tapping it opens the native install dialog, and the button disappears once installed. On **iOS Safari** the pill is always shown and expands Share → Add to Home Screen steps; tapping again collapses them. On **iOS Chrome/Edge/Firefox** the panel instead says to open in Safari first. Launch the installed app: the pill must **not** appear at all.
